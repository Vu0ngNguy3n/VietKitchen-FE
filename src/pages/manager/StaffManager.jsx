...
function StaffManager() {
    ...
    useEffect(() => {
        ...
        .then(res => {
            const data = res.data.result;
            setListEmployees(data.results);
            setListEmployeesDisplay(data.results);
            setTotalStaffs(data.totalItems);
            console.log(data);
            setSearch('');
        })
        .catch(err => {
    ...
    const handleClick = (page) => {
        if(page > 0 && page <= (totalStaffs / size + 1)){
            setCurrentPage(page);
        }
                        ...
                        <div className="relative overflow-x-auto shadow-md sm:rounded-lg mt-10">
                            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                             ...
                             <nav className="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4" aria-label="Table navigation">
                                <span className="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">Hiển thị <span className="font-semibold text-gray-900 dark:text-white">{1 + size*(currentPage-1)}-{(size + size*(currentPage-1) <= totalStaffs ?size + size*(currentPage-1) : totalStaffs)}</span> trong <span className="font-semibold text-gray-900 dark:text-white">{totalStaffs} </span>nhân viên</span>
                                <ul className="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
                                    ...
                                    {Array.from({ length: (totalStaffs%size >0 ? totalStaffs/size+1 : totalStaffs/size) }).map((_, index) => (
