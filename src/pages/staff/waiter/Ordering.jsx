  ...
  const addDish = (message) => {
    if (client && connected) {
      client.publish({
        ...
        body: JSON.stringify(message), // Chuyển đổi message thành chuỗi JSON trước khi gửi đi
      });
    } else {
          ...
          <div className="flex justify-center">
            ...
            >
              {areaList?.map((area, index) => (
                <option
                  value={area?.id}
                  selected={areaId === area?.id ? true : false}
                  key={index}
                >
                  {area?.name}
                </option>
            ...
            >
              {tableList?.map((table, index) => (
                <option
                  value={table?.id}
                  selected={tableId === table?.id ? true : false}
                  key={index}
                >
                  {table?.name}
                </option>
